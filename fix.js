...
exports.createServer = function(opt, func) {
  const server = new Server(opt, func);
  return server;
};
...
function Server(opt, func) {
  ...
  this.server = net.createServer(function(stream) {
    ...
    function SessionEmitterObj() {
      ...
      this.write = function(data) {
        session.p.pushOutgoing({data: data, type: 'data'});
      };
    }
    ...
    stream.on('end', function() {
      session.sessionEmitter.emit('end');
    });
  ...
  this.write = function(targetCompID, data) {
    self.sessions[targetCompID].write({data: data, type: 'data'});
  };
  this.logoff = function(targetCompID, logoffReason) {
    self.sessions[targetCompID].write({data: {35: 5, 58: logoffReason}, type: 'data'});
  };
  this.kill = function(targetCompID, reason) {
...
function Client(fixVersion, senderCompID, targetCompID, opt) {
  ...
  this.write = function(data) {
    self.p.pushOutgoing({data: data, type: 'data'});
  };
  this.connect = function(port, host, callback) {
    ...
    self.stream.on('data', function(data) {
      self.p.pushIncoming({data: data, type: 'data'});
    });
